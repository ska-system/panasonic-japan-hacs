...
class ConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_callback(
        ...
        try:
            ...
            member_id = user_info.get("member_user_id")
            if not member_id:
                errors["base"] = "invalid_token"
                return self.async_show_form(
                    step_id="callback",
            ...
            # Contextにトークン・ユーザー情報を保持して識別子入力ステップへ遷移
    ...
    async def async_step_identifier(
        ...
        return self.async_create_entry(
            title=f"CLUB Panasonic: {identifier}",
            data=entry_data,
        )
    ...
    async def async_step_reconfigure_callback(
        ...
        try:
            ...
            return self.async_update_reload_and_abort(
                existing_entry,
                title=f"CLUB Panasonic: {current_identifier}",
                data=new_data,
            )
